...
def build_args():
    ...
    parser.add_argument("--data_root_dir", type=str, default="/media/HDD2/VITON/StableVITON_git/StableVITON/test")
    parser.add_argument("--repaint", action="store_true")
    parser.add_argument("--unpair", action="store_true")
    parser.add_argument("--save_dir", type=str, default="./samples")

    parser.add_argument("--denoise_steps", type=int, default=70)
...
def run(args):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        for sample_idx, (x_sample, fn,  cloth_fn) in enumerate(zip(x_samples, batch['img_fn'], batch["cloth_fn"])):
            ...
            to_path = opj(save_dir, f"{fn.split('.')[0]}**{cloth_fn.split('.')[0]}.png")
