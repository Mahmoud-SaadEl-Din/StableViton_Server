...
class VITONHDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        agn = imread(opj(self.drd, "agnostic-v3.2", self.im_names[idx]), self.img_H, self.img_W)
        agn_mask = imread(opj(self.drd, "agnostic-mask", self.im_names[idx]), self.img_H, self.img_W, is_mask=True, in_inverse_mask=True) #self.im_names[idx].replace(".png", "_mask.png")
        cloth = imread(opj(self.drd, "cloth", self.c_names[self.pair_key][idx]), self.img_H, self.img_W)
